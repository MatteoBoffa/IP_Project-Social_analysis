...
if __name__ == '__main__':
	...
	base_dataFrame=pd.read_csv("./IGinQuadrato.csv")
	counters=base_dataFrame["count"]
	base_dataFrame=base_dataFrame.drop(["name","count","ID_QU_C_SI","TAVOLA_BN","TAVOLA_CO"],axis=1)
	...
	for i in range(0,len(base_dataFrame_scaled)-1):
		dist=np.linalg.norm(base_dataFrame_scaled[i]-base_dataFrame_scaled[i+1])
	...
	e_t=[1.5]#np.arange(0.7,1.6,0.1) #THESE ARE DEGREES
	M_t=[11]#np.arange(1,32,5)
	...
	for e in e_t:
		for M in M_t:
			...
			db_clusters=clus.DBSCAN(eps=e, min_samples=M).fit(base_dataFrame_scaled,counters)
	...
	for k, col in zip(unique_labels, colors):
		...
		if k==0:
			...
			print()
			c+=[counters[it] for it in range(len(counters)) if lab[it]==k and\
						core_samples_mask[it]==True]
	...
	df2=pd.DataFrame(excel_file,columns=["lat","lng","label","counters"])
